...
public:
  ...
  // ctor for a trade
    Trade<T>(){};
  Trade(const T &_product, string _tradeId, string _book,double price, long _quantity, Side _side);
...
template<typename T>
class TradeServiceListener :public ServiceListener<Trade<T>>
{
public:
...
template<typename T>
class TradeConnector: public Connector<Trade<T>>
{
public:
...
template<typename T>
Trade<T>::Trade(const T &_product, string _tradeId, string _book, double price, long _quantity, Side _side) :
  product(_product)
{
  tradeId = _tradeId;
  book = _book;
